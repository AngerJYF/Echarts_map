...
function setActiveView(targetChart, options, params) {
	...
	if (options.visualMap) {
		...
		options.visualMap.itemHeight = 180 * screenScale;
		options.visualMap.itemWidth = 30 * screenScale;
	}
	...
	if (isMainView) {
		...
		if (options.series) {
			options.series.forEach(function(x, i) {
				...
				if (x.effect) {
					options.series[i].effect.symbolSize = 6 * screenScale;
				}
			})
		}
	}
...
function featureLayout(containter, title, data) {
	...
	for (var pageIndex = 0; pageIndex < pages; pageIndex++) {
		...
		items.forEach(function(item) {
			...
			var itemContainer = $("<div class='pkx_item'><img src='" + item.src + "' alt='" + item.title + "' /><h3>" + item.title + "</h3></div>");
			...
			addSeries : function(series, res) {
				...
				for ( var schoolType in schoolData) {
					...
					series.push({
						...
						coordinateSystem : 'geo',
						zlevel : 2,
						symbolSize : schoolType == '双师学校' ? 1 : 2 * screenScale,
						rippleEffect : {
							brushType : 'stroke'
						},
						label : {
...
mapCity.on('click', function(params, ev) {
	...
	/*refreshvisualMap(mapCity, function(x) {
		return x;
	})*/
});
